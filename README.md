...
- 📊 **实时进度网页**：本地打开 `http://127.0.0.1:7788` 即看实时进度、当前视频、本条进度条、日志。`install-autostart.ps1` 会安装看门狗，进程异常退出后自动拉起。
- 🔑 **账号密码登录**：一键登录，Cookie 持久化，无需反复登录；也支持扫码。
- ⏸ **可暂停 / 继续（前端按钮）**：Dashboard 一键暂停，随时关电脑/干别的，重启自动续跑。`stop-windows.bat` 可彻底停止服务。
...
| 文件 | 作用 |
| --- | --- |
| `start-windows.bat` | 双击：后台启动刷课 + 看门狗 + 自动打开进度页 `http://127.0.0.1:7788` |
| `stop-windows.bat` | 双击：停止刷课与看门狗 |
| `install-autostart.ps1` | 设置「登录自启 + 看门狗」：登录 Windows 后自动运行，异常退出会自动恢复（无需管理员） |
| `uninstall-autostart.ps1` | 取消开机自启 |
| `start.sh` | Linux / macOS：`bash start.sh` 后台启动 |

> ✅ **推荐开启常驻模式**：先用 `node start.mjs --url "<课程URL>"` 完成登录与课程设置，然后运行一次 `install-autostart.ps1`。
> 之后每次登录 Windows，脚本都会自动启动；看门狗会持续检查 `http://127.0.0.1:7788`，进程崩溃或网页无响应时自动拉起。
>
> 如果暂时没有开启自启，重启后双击 **`start-windows.bat`** 即可恢复。诊断信息见 `watchdog.log`。
> 启动时会**自动打开浏览器**到进度页；直接运行命令时可用 `--no-open` 关闭自动打开。
## 🧭 常见用法
...
- **下次开机快速续跑**：已运行过 `install-autostart.ps1` 时会自动恢复；也可双击 `start-windows.bat`。脚本会记住课程，**自动跳过已完成的视频**，从没看完的接着刷。
